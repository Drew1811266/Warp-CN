...
impl AgentNotificationsModel {
            ...
            CLIAgentSessionsModelEvent::StatusChanged {
                ...
                CLIAgentSessionStatus::Success => {
                    let title = session_context
                        .display_title()
                        .unwrap_or_else(|| format!("{} 已完成", agent.display_name()));
                    let message = match agent {
                        CLIAgent::Codex => "来自 Codex 的通知",
                        _ => "任务已完成。",
                    };
                ...
                CLIAgentSessionStatus::Blocked { message } => {
                    let title = session_context
                        .display_title()
                        .unwrap_or_else(|| format!("{} 需要处理", agent.display_name()));
                    let metadata = TerminalViewMetadata::lookup(*terminal_view_id, ctx);
                    self.add_notification(
                        title,
                        message
                            .clone()
                            .unwrap_or_else(|| "正在等待输入。".to_owned()),
    ...
    fn handle_history_event_for_mailbox(
        ...
        let title = latest_query.unwrap_or_else(|| "Agent 任务".to_owned());
        let metadata = TerminalViewMetadata::lookup(terminal_view_id, ctx);
