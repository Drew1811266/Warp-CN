...
const NO_SECRET_LABEL: &str = "从环境继承密钥";
...
impl AuthSecretSelector {
    ...
    fn handle_secret_deleted(
        ...
        if removed_pending {
            ...
            let message = format!("API 密钥“{name}”已删除。");
    ...
    fn handle_secret_deletion_failed(
        ...
        if removed_pending {
            let window_id = ctx.window_id();
            let message = format!("删除 API 密钥“{name}”失败：{error}");
            ToastStack::handle(ctx).update(ctx, |ts, ctx| {
        ...
        AuthSecretFetchState::Loaded(secrets) => {
            for secret in secrets {
                ...
                let fields = MenuItemFields::new(secret.name.clone())
                    ...
                    .with_right_side_icon_a11y_label(format!("删除 API 密钥 {}", secret.name))
