# zh-Hans Localization Phase 62

Date: 2026-06-02 Asia/Shanghai

## Goal

Produce RC13 after the post-RC12 source, maintenance, visual-asset, and public-RC blocker passes.
...
nice -n 10 python3 script/zh_export_locale.py --format json > /tmp/warp-cn-zh-Hans-locale-rc13.json
python3 -m json.tool /tmp/warp-cn-zh-Hans-locale-rc13.json > /tmp/warp-cn-zh-Hans-locale-rc13.pretty.json
nice -n 10 python3 script/zh_export_locale.py --format yaml > /tmp/warp-cn-zh-Hans-locale-rc13.yaml
...
Phase 62 is accepted as `qualified-rc13-gate`.

The phase is qualified because the manifest and glossary gates pass, the final dry-run is clean, JSON/YAML exports complete, JSON parses, Python tests pass, formatting and diff checks pass, low-concurrency `cargo check -j 2 -p warp` passes, and the RC13 decision keeps source localization progress separate from visual-asset and public-RC external-state blockers.
